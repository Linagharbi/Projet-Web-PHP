...
<?php
 if (isset($_GET['supp']))
 { 
$repp = $bdd->query('SELECT * FROM utilisateur WHERE ID_utilisateur='.$_GET['id']); 
$donnees3 = $repp->fetch(); 
$reqq = $bdd->prepare('INSERT INTO archive (ID_utilisateur, nom, prenom, email, password, date_naissance,role,login) VALUES (:ID_utilisateur, :nom, :prenom, :email, :password,:date,:role,:login)');
    $reqq->execute(array(
			"ID_utilisateur" => $donnees3['ID_utilisateur'],
            "nom" => $donnees3['nom'], 
			"prenom" => $donnees3['prenom'], 
			"email" => $donnees3['email'],
            "password" => $donnees3['password'],
            "date" => $donnees3['date_naissance'],
			"role"=>$donnees3['role'],
			"login"=>$donnees3['login']
            ));

$count=$bdd->prepare('DELETE FROM utilisateur WHERE ID_utilisateur='.$_GET['id']);
$count->execute();
                           ...
                           <div class="notify"> <span class="heartbit"></span><span class="point"></span>  </div>
							...
							<?php 
                                ...
                                <ul>
                                    ...
                                    <li>
                                        <a class="nav-link text-center" href="notification.php"> <strong>Voir Tous Les Notifications</strong> <i class="fa fa-angle-right"></i> </a>
                                    </li>
                                </ul>
										<?php } ?>
                                    ...
                                    <li>
                                        <div class="message-center">
											<?php 
														...
														$rep = $bdd->query('SELECT * FROM message WHERE flag_lu=0 AND flag_arch=0 AND (id_dest='.$donnees11['ID_utilisateur'].')GROUP BY (ID_message) DESC ') ;
														
														while ($donnees111 = $rep->fetch())
															{
														?>
											 <a href="email.php">                                              
											<div class="mail-contnet">
												<?php  
																$rep1 = $bdd->query('SELECT * FROM utilisateur WHERE ID_utilisateur='.$donnees111['id_exp']) ;
															$donnees0 = $rep1->fetch();
															?>
                                                    <h5><?php echo $donnees0['prenom'] ; echo ' ' ; echo $donnees0['nom'] ;  ?></h5> <span class="mail-desc"><?php echo $donnees111['objet'] ; ?></span> <span class="time"><?php echo $donnees111['date_env'] ; ?> </div>
                                          </a>
                                ...
                                <ul class="dropdown-user">
                                    <li>
                                        <div class="dw-user-box">
                                            ...
                                            <div class="u-text">
                                                <h5><?php echo $donnees11['prenom']; echo' ' ; echo $donnees11['nom']; ?></h5>
                                                <p class="text-muted"><?php echo $donnees11['email']; ?></p><a href="profil.php" class="btn btn-rounded btn-danger btn-sm">Voir Profil</a></div>
                                        </div>
                                    </li>
            ...
            <div class="scroll-sidebar">
                ...
                <div class="user-profile">
                    ...
                    <!-- User profile text-->
                      <div class="profile-text">
                        ...
                        <h5><?php echo $donnees11['prenom']; ?></h5>
						<?php 
						$rep0 = $bdd->query('SELECT * FROM role WHERE ID_role='.$donnees11['role']); 
                ...
                <nav class="sidebar-nav">
                       ...
                       <p>&nbsp;
                    	...
                    	<?php
						if( ($donnees11['role'] == "1" ) || ($donnees11['role'] == "2" ))
						...
						<?php
                     	if (($donnees11['role'] == "2" ) || ($donnees11['role'] == "3" ))

						{ echo ' <li> <a class="has-arrow waves-effect waves-dark" href="taches.php" aria-expanded="false"><i class="mdi mdi-chart-bubble"></i><span class="hide-menu">Gestion des taches </span></a>
        ...
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-themecolor">Gestion des Employés</h3>
                </div>
                                    ...
                                    <table id="demo-foo-addrow" class="table m-t-30 table-hover contact-list" data-page-size="5">
                                        <thead>
										 ...
										 <?php
												...
												$req1 = $bdd->query('SELECT * FROM utilisateur ');
                                            ...
                                            <tr>
												...
												<td><?php echo $donnees1['nom']; ?></td>
												<td><?php echo $donnees1['prenom']; ?></td>
												<td><?php echo $donnees1['date_naissance']; ?></td>
												<td><?php echo $donnees1['email']; ?></td>
												
												
												<?php 
												
											$rep00 = $bdd->query('SELECT * FROM role where ID_role='.$donnees1['role'] ); 
											$donnees00 = $rep00->fetch();
												if ($donnees1['role'] == "1")
												{ echo '	
												<td><span class="label label-danger">'; echo $donnees00['type']; ?></span></td>
												<?php  } if($donnees1['role'] == "2")
												{ echo '	
												<td><span class="label label-info">';  echo $donnees00['type']; ?></span></td> 
												<?php  } if($donnees1['role'] == "3")
												{ echo '	
												<td><span class="label label-warning">';  echo $donnees00['type']; ?></span></td> 
												<?php  } ?>
													<td hidden ><?php echo $donnees1['login']; ?></td>
                                                 <td>
												<?php echo '<a href="modifieremp.php?id='.$donnees1["ID_utilisateur"].'">
                                                <button type="button" class="btn btn-sm btn-icon btn-pure btn-outline settings-row-btn" data-toggle="tooltip" data-original-title="Modifier"><i class="ti-marker-alt" ></i></button></a>
												
												<a href="employes.php?supp&id='.$donnees1["ID_utilisateur"].'" onclick="return supprimer();">
                                        ...
                                        <tfoot>
                                            <tr>
                                                <td colspan="2">
                                                    <button type="button" class="btn btn-info btn-rounded" data-toggle="modal" data-target="#add-contact">Ajouter Employé</button>
                                                </td>
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form class="form-material" method="post" action="saisie.php" >
                                                                    <div class="form-group">
                                                                        <div class="col-md-12 m-b-20">
																			<label for="exampleInputuname2">Nom </label>
                                                                            <input type="text" class="form-control" name="nom"  placeholder="Nom" required="required"  > </div>
                                                                            <div class="col-md-12 m-b-20">
																			<label for="exampleInputuname2">Prenom</label>
                                                                            <input type="text" class="form-control" name="prenom" placeholder="Prenom" required=""> </div>
                                                                        <div class="col-md-12 m-b-20">
																		<label for="exampleInputuname2">Login</label>
                                                                            <input type="text" class="form-control" name="login" placeholder="Login" required=""> </div>
                                                                        <div class="col-md-12 m-b-20">
																		<label for="exampleInputuname2">Email</label>
                                                                            <input type="email" class="form-control" name="email" placeholder="Email" required=""> </div>
                                                                        <div class="col-md-12 m-b-20">
																		<label for="exampleInputuname2">Mot de passe</label>
                                                                            <input type="password" class="form-control" name="password" placeholder="Mot de passe"required="" > </div>
                                                                        <div class="col-md-12 m-b-20">
																		<label for="exampleInputuname2">Confirmer mot de passe</label>
                                                                            <input type="password" class="form-control" name="repeatpassword" placeholder="Confirmer mot de passe" required=""> </div>
                                                                        <div class="col-md-12 m-b-20">
																		<label for="exampleInputuname2">Date de naissance</label>
                                                                            <input type="date" class="form-control" name="date" placeholder="Date de naissance"> </div>
                                                                       <!-- button -->
																	   <div class="col-md-12 m-b-20">
																			<label for="exampleInputuname2">Role</label>
																		
																			<SELECT name="role" class="form-control"  placeholder="Role" style="font-size: 10pt; border-style: solid; border-width: 1px; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px">
																					<?php
							
																		$req02 = $bdd->prepare('SELECT * FROM role ');
																		$req02->execute() ;
																		while ($donnees02 = $req02->fetch())
																			{
																		?>
								
																<OPTION value="<?php echo $donnees02['ID_role'];?>"> <?php echo $donnees02['type']; ?></OPTION> 
																	<?php } ?>						
																	</SELECT>
																		</div>																	
																				</div>
