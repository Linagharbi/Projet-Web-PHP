...
$req0=$bdd->query('SELECT * FROM tache WHERE utilisateur="(SELECT ID_utilisateur from utilisateur)"');
$donnees0 = $req0->fetch(); 

if (isset($_GET['supp']))
{ if ( $donnees0['statut']=="Validée") 
                ...
                <nav class="sidebar-nav">
                       ...
                       <p>&nbsp;
						...
						?>
						 <?php
						if ($donnees1['role'] == "2" ) 
						{echo ' <li> <a class="has-arrow waves-effect waves-dark" href="suiviemp.php" aria-expanded="false" ><i class="mdi mdi-account-multiple"></i><span class="hide-menu">Suivi</span></a>  
                        </li> ';
						}
						?>
        ...
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-themecolor">suivi</h3>
                </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table id="demo-foo-addrow" class="table m-t-30 table-hover contact-list" data-page-size="5">
										...
										$res = $bdd->query('SELECT DISTINCT (utilisateur) FROM tache WHERE projet in (SELECT ID_projet FROM projet  
															where user='.$id_user.')');
												
												 
												while ($donnees01 = $res->fetch())
												{ 
													$req1 = $bdd->query('SELECT * FROM utilisateur WHERE ID_utilisateur='.$donnees01['utilisateur']);
													$donnees2 = $req1->fetch();
												 
												$req100 = $bdd->prepare('SELECT COUNT(*) AS nbre00 FROM tache WHERE statut="à faire" and utilisateur='.$donnees2['ID_utilisateur']);
										$req100->execute() ;
										$donnees100 = $req100->fetch();
										$nbret =$donnees100['nbre00'];
										
										//echo $nbret[$i];
										$req20 = $bdd->prepare('SELECT COUNT(*) AS nbre1 FROM tache WHERE statut="En Cours" and utilisateur='.$donnees2['ID_utilisateur']);
										$req20->execute() ;
										$donnees20 = $req20->fetch();
										$nbret1 =$donnees20['nbre1'] ;
										
										// echo $nbret1[$i].'</br>';
										$req30 = $bdd->prepare('SELECT COUNT(*) AS nbre2 FROM tache WHERE statut="Terminée" and utilisateur='.$donnees2['ID_utilisateur']);
										$req30->execute() ;
										$donnees30 = $req30->fetch();
										$nbret2= $donnees30['nbre2'] ;
										
										//echo $nbret2[$i].'</br>';
										$req40 = $bdd->prepare('SELECT COUNT(*) AS nbre3 FROM tache WHERE statut="Validée" and utilisateur='.$donnees2['ID_utilisateur']);
										$req40->execute() ;
										$donnees40 = $req40->fetch();
										$nbret3= $donnees40['nbre3'] ;
										
										//echo $nbret3[$i].'</br>';;
										?>
                                            <tr>
                                                <td hidden><?php echo $donnees2['ID_utilisateur']; ?></td>
												...
												<td>
												<i class="btn btn-sm btn-circle btn-outline-danger" data-toggle="tooltip" data-original-title="Tâche à faire :<?php  echo '  ' ; echo $nbret ; ?>" > <i class=" mdi mdi-alarm-check " > </i></i>
												<i class="btn btn-sm btn-circle btn-outline-warning"  data-toggle="tooltip" data-original-title="Tâche En cours :<?php  echo '  ' ; echo $nbret1 ; ?> "><i class="ti-reload" ></i></i>
												<i class="btn btn-sm btn-circle btn-outline-info"  data-toggle="tooltip" data-original-title="Tâche Terminée :<?php  echo '  ' ; echo $nbret2 ; ?>"><i class="icon-emotsmile" ></i></i>
												<i class="btn btn-sm btn-circle btn-outline-success"  data-toggle="tooltip" data-original-title="Tâche Validée :<?php  echo '  ' ; echo $nbret3 ; ?>"><i class=" ti-check" ></i></i>
