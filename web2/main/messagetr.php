                        ...
                        <div class="card">
                            <div class="row">
                                <div class="col-xlg-2 col-lg-3 col-md-4 ">
                                    <div class="card-body inbox-panel"><a href="message.php" class="btn btn-danger m-b-20 p-10 btn-block waves-effect waves-light">Nouveau message</a>
                                       <ul class="list-group list-group-full">
										<?php 
										$req00 = $bdd->prepare('SELECT COUNT(*) AS nbre FROM message WHERE id_dest='.$_SESSION['ID_utilisateur']);
											$req00->execute() ;
											$donnees00 = $req00->fetch();
											
										?>
                                            <li class="list-group-item active"> <a href="email.php"><i class="mdi mdi-gmail"></i> Boîte de réception </a><span class="badge badge-success ml-auto"><?php echo $donnees00['nbre'] ; ?> </span></li>
                                          <?php  ?>                                           
                                         
                                            <li class="list-group-item ">
                                                <a href="emailenv.php"> <i class="mdi mdi-file-document-box"></i> Messages envoyés </a>
                                            </li>
                                            <li class="list-group-item">
                                                <a href="javascript:void(0)"> <i class="mdi mdi-delete"></i> Corbeille </a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="col-xlg-10 col-lg-9 col-md-8 bg-light-part b-l">
								 <form action="contenu.php" method="post">
                                    <div class="card-body">
										...
										<?php 
										$rep = $bdd->query('SELECT * FROM utilisateur WHERE ID_utilisateur='.$_SESSION['ID_utilisateur']); 
										$donnees11 = $rep->fetch(); 
										?>
                                            <input name="expediteur" value="<?php echo $donnees11['ID_utilisateur'] ; ?>" hidden class="form-control">
                                        </div>
                                        <div class="form-group">
										
										<SELECT name="destinataire" class="form-control" placeholder="à:" align="center" size="1" style="font-size: 10pt; border-style: solid; border-width: 1px; padding-left: 4px; padding-right: 4px; padding-top: 1px; padding-bottom: 1px">	
								<?php
								
								$reqq = $bdd->prepare('SELECT * FROM utilisateur');
								$reqq->execute() ;
								while ($donneesq = $reqq->fetch())
								{
								?>
								
                                  <OPTION><?php echo $donneesq['email'];?></OPTION> 
								 <?php } ?>						
								 </SELECT>
										
									
                                            <!--<input name="destinataire"  class="form-control" placeholder="à:"> -->
                                        </div>
										<?php
								
									$reqa = $bdd->prepare('SELECT * FROM message WHERE ID_message='.$id);
									$reqa->execute() ;
									$donneesqa = $reqa->fetch()
								
								?>
										
                                        <div class="form-group">
                                            <input name="objet" class="form-control" value="<?php echo $donneesqa['objet']; ?>">
                                        </div>
                                        <div class="form-group">
                                            <input name="sujet" class="textarea_editor form-control" value="<?php echo $donneesqa['sujet']; ?>" rows="15" >
                                        </div>
                                        <h4><i class="ti-link"></i> Joindre des fichiers</h4>
                                      
                                            <div class="fallback">
                                                <input name="file" type="file" multiple />
                                            </div>
                                      
                                        <button name="envoyer" type="submit" class="btn btn-success m-t-20"><i class="fa fa-envelope-o"></i> Envoyer</button>
                                        <button class="btn btn-inverse m-t-20"><i class="fa fa-times"></i> Annuler</button>
                                    </div>
									 </form>
                                </div>
                            </div>
